...
function sc_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'sc' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'sc' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'sc' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author -->

				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'sc' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
					<?php edit_comment_link( __( 'Edit', 'sc' ), '<span class="edit-link">', '</span>' ); ?>
				</div><!-- .comment-metadata -->

				<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'sc' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
...
endif; // ends check for sc_comment()

if ( ! function_exists( 'sc_the_attached_image' ) ) :
/**
 * Prints the attached image with a link to the next attached image.
 */
function sc_the_attached_image() {
	$post                = get_post();
	$attachment_size     = apply_filters( 'sc_attachment_size', array( 1200, 1200 ) );
