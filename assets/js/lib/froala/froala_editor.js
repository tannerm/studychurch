...
!function ($) {
  ...
  Editable.prototype.uploadPastedImages = function () {
    ...
    else if (this.options.imageUpload) {
    ...
    for (i = 0; i < html.length; i++) {
      ...
      if (chr == '<') {
        ...
        if (j !== -1) {
          ...
          // Handle IE comments.
          if (tag_name.indexOf('!') === 0 && dom.length && dom[dom.length - 1] != tag) {
            dom.push(tag);
            i = j;
            continue;
          }
  ...
  Editable.prototype.cleanupLists = function () {
    this.$element.find('ul, ol').each ($.proxy(function (index, list) {
      var $list = $(list);

      if (this.parents($(list), 'ul, ol').length > 0) return true;
      ...
      if ($list.find('.close-ul, .open-ul, .close-ol, .open-ol, .open-li, .close-li').length > 0) {
        ...
        oldHTML = oldHTML.replace(new RegExp('<li></li>', 'g'), '');
    ...
    this.$element.find('li > ul, li > ol').each ($.proxy(function (idx, lst) {
      var prev = $(lst).parent().get(0).previousSibling;
      if (this.isFirstSibling(lst)) {
    ...
    if (attrs) {
      ...
      for (var i = 0; i < attrs.length; i++) {
        ...
        if (!r.test(att.nodeName)) {
          ...
          node.setAttribute(
            att.nodeName,
            att.nodeValue.replace(/</gi, '&lt;').replace(/>/gi, '&gt;')
          )
        }
      }
    }
  ...
  Editable.prototype.clean = function (html, allow_id, clean_style, allowed_tags, allowed_attrs) {
    ...
    // Take care of HTML comments.
    html = html.replace(/<!--([.\s\w\W]*?)-->/gi, function (str, a1) {
      return '<!--' + a1.replace(/</g, '[[').replace(/>/g, ']]') + '-->';
    });
  ...
  Editable.prototype.getHTML = function (keep_markers, add_fr_tag, remove_verifier) {
    ...
    this.$element.find('pre').each ($.proxy(function (index, pre) {
      ...
      var new_html = old_html.replace(/\&nbsp;/gi, ' ').replace(/\n/gi, '<br>');
  ...
  Editable.prototype.addMarkersAtEnd = function ($element) {
    if ($element.find('.fr-marker').length > 0) return false;

    var node = $element.get(0);
    var contents = $(node).contents();
    while (contents.length && this.valid_nodes.indexOf(contents[contents.length - 1].tagName) >= 0) {
...
(function ($) {
  ...
  $.Editable.prototype.indent = function (outdent, reposition) {
    ...
    for (var j = 0; j < elements.length; j++) {
      if ($(elements[j]).parentsUntil(this.$element, 'li').length > 0) {
        elements[j] =  $(elements[j]).closest('li').get(0);
      }
    }
  ...
  $.Editable.prototype.inlineStyle = function (prop, cmd, val) {
    ...
    if (this.browser.webkit) {
      ...
      this.$element.find('[style]').each (function (index, span) {
        var $span = $(span);

        if (hasFontSizeSet($span)) {
          $span.attr('data-font-size', $span.css('font-size'));
          $span.css('font-size', '');
        }
      })
    }
    ...
    // Restore font size.
    if (this.browser.webkit) {
      this.$element.find('[data-font-size]').each (function (index, span) {
        var $span = $(span);

        $span.css('font-size', $span.attr('data-font-size'));
        $span.removeAttr('data-font-size');
      })
    }
    ...
    // Insert link.
      ...
      if (!this.isLink) {
        ...
        if (this.options.linkText || this.text() === '') {
          this.insertHTML('<span class="f-marker" data-fr-verified="true" data-id="0" data-type="true"></span>' + (text || this.clean(original_url)) + '<span class="f-marker" data-fr-verified="true" data-id="0" data-type="false"></span>');
          this.restoreSelectionByMarkers();
        }
